...
class VerifyActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private var getUserToken: String? = null
    ...
    private fun getDataReg(){
        noTelpon = intent.extras.getString("nomorID")
        noTelpon2 = intent.extras.getString("nomorID2")
    ...
    private fun SetupVertificationCallBack(){
    ...
    private fun profileUser(){
        ...
        getUserToken = SharedPrefManager.getInstance(this@VerifyActivity).token

        //Data probadi dari User dan Provider
        val getUserID = getUser!!.uid
        val getNama = intent.extras.getString("namaID")
        val getNomor = getUser.phoneNumber
        val getStatus = "Saya bangga menggunakan aplikasi karya anak bangsa"
        val setData = dataUser(getNama.toLowerCase(), getUserToken.toString(), getNama, "default",
                  "default", getNomor.toString(), getStatus)
        reference.child("Users").child(getUserID).setValue(setData).addOnCompleteListener { task: Task<Void> ->
        ...
        //Meyimpan data user pada SharedPreference
        SharedPrefManager.getInstance(this).storeDataProfil(getNama, noTelpon2!!, getStatus)
    }
